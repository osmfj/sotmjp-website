...
from sotm.models import SotMTutorialProposal
...
class Command(NoArgsCommand):
    ...
    def handle_noargs(self, **options):
        ...
        if not req.raise_for_status():
            ...
            # CTE ID: SotMTutorialProposal
            tutorials = {}
            # SotMTutorialProposal: [emails,]
            registrant_data = {}
            # pop header row
            reader.next()
            for row in reader:
                if row:
                    ...
                    if tut_id in tutorials:
                        ...
                        tutorial = SotMTutorialProposal.objects.get(proposalbase_ptr=tut_id)
                        if max_attendees:
